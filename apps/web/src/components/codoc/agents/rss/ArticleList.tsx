...
interface ArticleListProps {
  items: Article[];
  itemAction?: (item: Article, index: number) => ViewAction;
}
...
export function ArticleList({ items, itemAction }: ArticleListProps) {
        ...
        return (
          ...
          >
            ...
            <div className="flex items-start gap-2">
              <div
                className={`flex-1 min-w-0 ${isRead ? "" : "font-medium"}`}
              >
                <span className="flex items-baseline gap-2 flex-wrap">
                  {item.feedTitle && (
                    <span
                      className={`text-xs rounded px-1.5 py-0.5 ${feedTagColor(item.feedTitle)}`}
                    >
                      {item.feedTitle}
                    </span>
                  )}
